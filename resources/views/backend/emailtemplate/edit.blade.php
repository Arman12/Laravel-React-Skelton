    ...
    <div class="col-lg-12 grid-margin stretch-card">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <form action="{{ route('email.update', ['emailTemplate' => $emailTemplate]) }}" method="POST" class="form-sample">
            @csrf
            <div class="row">
              <div class="col-md-6">
                <div class="form-group row">
                  ...
                  <div class="col-sm-9">
                    <input type="text" name="title" value="{{ $emailTemplate->title }}" class="form-control" />
                    @error('title')
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group row">
                  ...
                  <div class="col-sm-9">
                    <textarea name="description"  class="form-control" id="editor1">{{ $emailTemplate->description }}</textarea>
                    @error('description')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                  </div>
                  ...
                  <div class="col-sm-4">
                    <div class="form-check">
                      <label class="form-check-label">
                        <input type="radio" class="form-check-input" name="status" id="status1" value="active" {{ $emailTemplate->status == 'active' ? 'checked' : '' }}> Active </label>
                    </div>
                  </div>
                  <div class="col-sm-5">
                    <div class="form-check">
                      <label class="form-check-label">
                        <input type="radio" class="form-check-input" name="status" id="status2" value="disable" {{ $emailTemplate->status == 'disable' ? 'checked' : '' }}> Disable </label>
                    </div>
                  </div>
                  @error('status')
                    <span class="text-danger">{{ $message }}</span>
